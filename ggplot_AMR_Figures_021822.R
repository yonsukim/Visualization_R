...
(p <- ggplot(df_AMR, aes(x = boxOdds, y =yAxis)) + 
    ...
    geom_errorbarh(data = filter(df_AMR, yAxis=="0.4" & grouping=="African Americans"), aes(xmax = boxCIHigh, xmin = boxCILow), size = .05, height = .15, color = "gray40") +
    ...
    geom_errorbarh(data = filter(df_AMR, yAxis=="0.3" & grouping=="Whites"), aes(xmax = boxCIHigh, xmin = boxCILow), size = .05, height = .15, color = "gray40", linetype = "dashed") +
